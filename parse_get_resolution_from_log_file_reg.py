...
def save_to_csv(data, output_file, target):
    with open(output_file, 'a', newline='') as csvfile:
        fieldnames = ['Target','Regression Map Alone (Masked)', 'Regression Map Alone (Unmasked)', 'Regression FSC_0143 (Masked)', 'Regression FSC_0143 (Unmasked)','Regression FSC_05 (Masked)', 'Regression FSC_05 (Unmasked)']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        # writer.writeheader()
        writer.writerow({'Target' : target,
                        'Regression Map Alone (Masked)': data['d99'].get('masked', ''),
                        'Regression Map Alone (Unmasked)': data['d99'].get('unmasked', ''),
                        'Regression FSC_0143 (Masked)': data.get('fsc_0.143', {}).get('masked', ''),
                        'Regression FSC_0143 (Unmasked)': data.get('fsc_0.143', {}).get('unmasked', ''),
                        'Regression FSC_05 (Masked)': data.get('fsc_0.5', {}).get('masked', ''),
                        'Regression FSC_05 (Unmasked)': data.get('fsc_0.5', {}).get('unmasked', '') })
...
def main():
    ...
    output_csv  = "/curr_regression_fsc_values.csv"
    ...
    for m in map_directories:
        ...
        log_file = f"{density_map_path}/reg_mtriage.out"
