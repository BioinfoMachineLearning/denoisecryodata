...
def save_to_csv(data, output_file, target):
    with open(output_file, 'a', newline='') as csvfile:
        fieldnames = ['Target','Experimental Map Alone (Masked)', 'Experimental Map Alone (Unmasked)', 'Experimental FSC_0143 (Masked)', 'Experimental FSC_0143 (Unmasked)','Experimental FSC_05 (Masked)', 'Experimental FSC_05 (Unmasked)']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        # writer.writeheader()
        writer.writerow({'Target' : target,
                        'Experimental Map Alone (Masked)': data['d99'].get('masked', ''),
                        'Experimental Map Alone (Unmasked)': data['d99'].get('unmasked', ''),
                        'Experimental FSC_0143 (Masked)': data.get('fsc_0.143', {}).get('masked', ''),
                        'Experimental FSC_0143 (Unmasked)': data.get('fsc_0.143', {}).get('unmasked', ''),
                        'Experimental FSC_05 (Masked)': data.get('fsc_0.5', {}).get('masked', ''),
                        'Experimental FSC_05 (Unmasked)': data.get('fsc_0.5', {}).get('unmasked', '') })
...
def main():
    dataset_dir = "/experimental_pdb"
    output_csv  = "/curr_experimental_fsc_values.csv"
    ...
    for m in map_directories:
        ...
        log_file = f"{density_map_path}/exp_mtriage.out"
